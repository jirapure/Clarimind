...
google = oauth.remote_app(
    'google',
    consumer_key='238530097962-jblgf4eorfglgivjok29gfu32lcm4q63.apps.googleusercontent.com',
    consumer_secret='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',
...
def has_profanity(text):
    ...
    abusive = ["sexy", "sex", "porn", "xxx", "fuck", "nude","die","death","murder","suicide","hell"]
    return any(keyword in text.lower() for keyword in abusive)

def generate_decision(scenario):
    options = re.split(r",|\bor\b", scenario.strip())
    all_results = []

    for decision_number, option in enumerate(options, 1):  # Use enumerate
        retries = 2
        while retries > 0:
            messages = [
                ...
                {"role": "user", "content": f"Provide me 3 concise pros and 3 concise cons for {option}."}
            ]
            ...
            try:
                ...
                if "Pros:" in content and "Cons:" in content:
                    decision = options[decision_number - 1].strip()  # Access original option from options list
                    pros_cons_text = content.split("Pros:")
                    pros_cons_text = pros_cons_text[1].strip() if len(pros_cons_text) > 1 else ""
